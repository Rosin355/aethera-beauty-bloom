...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const accessUrl = `${Deno.env.get('SUPABASE_URL')?.replace('.supabase.co', '.lovable.app') || 'http://localhost:5173'}/welcome?token=${access_token}`;
    ...
    const emailResponse = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: '4 Elementi Italia <benvenuto@4elementi.it>',
        to: [email],
        subject: 'Benvenuto/a in 4 Elementi Italia! 🎉',
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: linear-gradient(135deg, #1B1B1B, #2D2D2D); color: white; border-radius: 12px; overflow: hidden;">
            <div style="padding: 40px 30px; text-align: center;">
              <h1 style="color: #6AA8B3; font-size: 28px; margin-bottom: 20px;">
              ...
              <div style="background: rgba(106, 168, 179, 0.1); padding: 20px; border-radius: 8px; margin: 30px 0;">
                ...
                <ul style="text-align: left; color: #CBD8D4; line-height: 1.6;">
                  ...
                  <li>✅ Accesso alla community riservata (presto!)</li>
                </ul>
              </div>
    ...
    */
    ...
    return new Response(JSON.stringify({ 
      success: true, 
      message: 'Welcome email sent successfully (simulated - configure RESEND_API_KEY to send real emails)',
